...
// Get poll statistics for the current user
...
router.post('/:sessionCode/vote', async (req, res) => {
  try {
    ...
    try {
      // Use the new increment method and save in a single operation
...
// Get all polls (public and user's private polls)
...
// Get current question data by session code
router.get('/:sessionCode/current', async (req, res) => {
  try {
    ...
    // Return only the necessary data for the current question
